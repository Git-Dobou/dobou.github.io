...
class EconomizeDetailScreen extends StatelessWidget {
  ...
  const EconomizeDetailScreen({Key? key, required this.economizeGoal}) : super(key: key);
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => AddEditEconomizeScreen(economizeGoal: economizeGoal),
                ),
              );
            },
          ),
              ...
              showDialog(
                ...
                builder: (BuildContext ctx) {
                  return AlertDialog(
                    ...
                    content: Text('Are you sure you want to delete this savings goal "\${economizeGoal.name}"?', style: textTheme.bodyLarge),
                    actions: <Widget>[
                        ...
                        onPressed: () {
                          ...
                          debtNotifier.deleteEconomize(economizeGoal.id).then((_){
                            scaffoldMessenger.showSnackBar(
                              SnackBar(content: Text('Savings Goal "\${economizeGoal.name}" deleted.'), backgroundColor: Colors.green)
                            );
        ...
        child: Card( // Themed
          ...
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: <Widget>[
                Center(child: Text(economizeGoal.name, style: textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold, color: colorScheme.primary))),
                ...
                Divider(),
                _buildDetailRow(context, 'Goal Amount', currencyFormat.format(economizeGoal.amount)),
                _buildDetailRow(context, 'Target Date', dateFormat.format(economizeGoal.date)),
                SizedBox(height: 16),
                Text('Progress: \${currencyFormat.format(currentSavedAmount)} / \${currencyFormat.format(economizeGoal.amount)} (\${(progress * 100).toStringAsFixed(1)}%)', style: textTheme.titleMedium),
                SizedBox(height: 8),
                ClipRRect(
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            title: Text('Transaction ID: \${ref.id}', style: textTheme.bodyMedium),
            trailing: IconButton(
